...
class HybridRetriever:
    ...
    def initialize_bm25(self, documents: Optional[List[Document]] = None):
        """
        Inicializa o índice BM25 com todos os documentos
        Deve ser chamado antes de usar estratégias que incluem BM25
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.vectorstore:
            try:
                all_data = self.vectorstore.get()
                stats["vectorstore_document_count"] = len(all_data.get("documents", []))
            except Exception as e:
                stats["vectorstore_document_count_error"] = str(e)
...
def get_retriever() -> HybridRetriever:
    ...
    Returns:
        HybridRetriever: Instância única do retriever
    
    Raises:
        FileNotFoundError: Se vectorstore não for encontrado
    
    Example:
        >>> from src.services.retrieval_service import get_retriever
        >>> retriever = get_retriever()
        >>> retriever.initialize_bm25()
        >>> results = retriever.retrieve("minha query", k=5)
    """
    global _retriever_instance
    
    if _retriever_instance is None:
        print("🔧 Criando instância singleton do HybridRetriever...")
        _retriever_instance = HybridRetriever()
        print("✅ Singleton do HybridRetriever criado")
...
def reset_retriever():
    """
    Reseta o singleton do retriever (útil para testes)
    
    AVISO: Use com cuidado em produção!
    """
    global _retriever_instance
    _retriever_instance = None
    get_retriever.cache_clear()  # Limpa cache do lru_cache
    print("🔄 Singleton do retriever resetado")
...
if __name__ == "__main__":
    ...
    print(f"   Mesma instância? {retriever1 is retriever2}")  # Deve ser True
    
    # Stats iniciais
    print(f"\n2️⃣ Stats iniciais: {retriever1.get_stats()}")
    
    # Inicializa BM25
    print("\n3️⃣ Inicializando BM25...")
    try:
        retriever1.initialize_bm25()
        print(f"   Stats após BM25: {retriever1.get_stats()}")
    except FileNotFoundError as e:
        print(f"   ⚠️  Erro: {e}")
        print("   Execute a ingestão de documentos primeiro")
    
    # Teste de busca vetorial
    if retriever1.vectorstore is not None:
        print("\n4️⃣ Testando busca vetorial...")
        query = "Qual o prazo para renovação de acreditação?"
        
        try:
            result = retriever1.retrieve(
                query, 
                k=5, 
            ...
            if result.documents:
                ...
                print(f"      Conteúdo: {result.documents[0].page_content[:200]}...")
